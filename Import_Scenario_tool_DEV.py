  ...
  # --- Aba 4: Gerenciamento de Produtos (NCM) ---
    with management_tabs[3]:
        ...
        if products:
            ...
            if search_query:
                filtered_products = {ncm: prod for ncm, prod in products.items() 
                                     if search_query.lower() in ncm.lower() 
                                     or search_query.lower() in prod.get("descricao", "").lower()}
            else:
                filtered_products = products
    
            if filtered_products:
                for ncm in list(filtered_products.keys()):
                    prod = filtered_products[ncm]
        ...
        if not data:
            ...
            if filial_selected in data:
                for scenario, config in data[filial_selected].items():
                    ...
                    # Cria o dicionário com os campos desejados:
                    costs[scenario] = {
                        "Valor FOB": valor_fob_usd,
                    ...
                    # Acrescenta os demais campos do cenário, se desejar manter outros dados
                    for field, conf in config.items():
                        if isinstance(conf, dict):
                            field_type = conf.get("type", "fixed")
                            rate_by_occupancy = conf.get("rate_by_occupancy", False)
