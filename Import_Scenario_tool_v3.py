...
if option == "Configuração":
    ...
    for main_tab, filial in zip(main_tabs, filial_names):
        with main_tab:
            ...
            scenario_list = list(data[filial].keys())
            scenario_to_remove = st.selectbox("Selecione um cenário para remover", ["Nenhum"] + scenario_list)
            if scenario_to_remove != "Nenhum" and st.button(f"Remover Cenário: {scenario_to_remove}"):
                del data[filial][scenario_to_remove]
                save_data(data)
                st.experimental_rerun()

            new_scenario = st.text_input("Adicionar Novo Cenário")
            if st.button("Adicionar Cenário") and new_scenario:
            ...
            for scenario in data[filial].keys():
                ...
                for field, value in data[filial][scenario].items():
                    ...
                    data[filial][scenario][field] = st.number_input(f"{field}", min_value=0, value=value, key=unique_key)
